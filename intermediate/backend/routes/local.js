...
router.get('/health', async (req, res) => {
    try {
        res.json({
            database: 'sqlite',
            application: 'smartslate-intermediate',
            status: 'ok',
            offlineReady: true,
            dbFile: db.dbPath
        });
...
router.post('/profile', async (req, res) => {
    ...
    try {
        ...
        console.log(`[SQLite Intermediate] Profile saved for UID: ${firebase_uid}`);
...
router.post('/notes', async (req, res) => {
    ...
    try {
        ...
        console.log(`[SQLite Intermediate] Note saved locally for UID: ${firebase_uid} (ID: ${note_id})`);
