...
const db = require('../database/sqlite.cjs');
...
router.get('/health', async (req, res) => {
    try {
        res.json({
            database: 'sqlite',
            application: 'smartslate-elementary',
            status: 'ok',
            offlineReady: true,
            dbFile: db.dbPath
        });
...
router.post('/profile', async (req, res) => {
    ...
    try {
        ...
        await db.run(
            ...
            [firebase_uid, student_id || '', name || 'Student', email || '', studentClass || '4', `Class ${studentClass || '4'}`, section || 'A']
        );

        console.log(`[SQLite Elementary] Profile saved for UID: ${firebase_uid}`);
...
router.post('/notes', async (req, res) => {
    ...
    try {
        ...
        console.log(`[SQLite Elementary] Note saved locally for UID: ${firebase_uid} (ID: ${note_id})`);
