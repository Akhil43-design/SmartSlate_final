...
function StudentHome() {
  ...
  useEffect(() => {
    ...
    const unsub = subscribeToAuthChanges(async (user) => {
      if (user) {
        console.log(`[AUTH] Home page session: ${user.uid} (${user.email})`);
        const stu = await getStudent(user.uid);
        if (stu) {
          const name = stu.name || (stu as any).fullName;
          const firestoreUid = stu.uid || user.uid;
          const uidMatch = firestoreUid === user.uid;
          console.log(`[HOME PROFILE]\nAUTH UID: ${user.uid}\nPROFILE UID: ${firestoreUid}\nPROFILE NAME: ${name}\nHOME GREETING: Hi, ${name}! 👋\nUID MATCH: ${uidMatch}`);
          if (uidMatch && name) {
            setStudentName(name);
          }
