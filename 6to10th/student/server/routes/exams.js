...
function calculateExamWindow(exam, now = new Date()) {
    ...
    if (exam.start_time && String(exam.start_time).includes('T')) {
        startDateTime = new Date(exam.start_time);
    } else {
        const startDateStr = exam.start_date || new Date().toISOString().split('T')[0];
        const startTimeStr = exam.start_time || '00:00';
        startDateTime = new Date(`${startDateStr}T${startTimeStr.length === 5 ? startTimeStr + ':00' : startTimeStr}`);
    }

    if (exam.end_time && String(exam.end_time).includes('T')) {
        endDateTime = new Date(exam.end_time);
    } else {
        const endDateStr = exam.end_date || exam.start_date || new Date().toISOString().split('T')[0];
        const endTimeStr = exam.end_time || '23:59';
        endDateTime = new Date(`${endDateStr}T${endTimeStr.length === 5 ? endTimeStr + ':00' : endTimeStr}`);
    }

    if (isNaN(startDateTime.getTime())) startDateTime = new Date(Date.now() - 3600000);
    if (isNaN(endDateTime.getTime())) endDateTime = new Date(Date.now() + 86400000);
...
router.post('/:id/start', authenticateToken, async (req, res) => {
    try {
        const examId = req.params.id;
        let exam = await get("SELECT * FROM exams WHERE id = ? OR id = ?", [examId, parseInt(examId, 10) || 0]);
        if (!exam) exam = await get("SELECT * FROM exams ORDER BY id DESC LIMIT 1");
        if (!exam) return res.status(404).json({ error: 'Exam not found.' });
...
// POST /api/exams/:id/draft - Autosave in-progress exam answers (MCQ or Handwriting strokes)
router.post('/:id/draft', authenticateToken, async (req, res) => {
    try {
        const examId = req.params.id;
        const { answers } = req.body;
        if (!answers) return res.status(400).json({ error: 'Answers payload required.' });

        const studentInfo = await resolveStudent(req.user);
        const sub = await get(
            "SELECT id, status FROM exam_submissions WHERE exam_id = ? AND (student_id = ? OR student_uid = ?)",
            [examId, studentInfo.studentId, req.user.id]
        );

        if (sub && sub.status !== 'in_progress') {
            return res.json({ success: true, status: sub.status, message: 'Exam already submitted.' });
        }
        ...
        await run(
            `INSERT INTO exam_submissions (exam_id, student_id, student_uid, answers, status)
             VALUES (?, ?, ?, ?, 'in_progress')
             ON CONFLICT(exam_id, student_id)
             DO UPDATE SET answers = excluded.answers WHERE status = 'in_progress'`,
            [examId, studentInfo.studentId, req.user.id, JSON.stringify(answers)]
        );

        res.json({ success: true, message: 'Draft autosaved successfully.' });
    } catch (err) {
        console.error('Autosave draft error:', err);
        res.status(500).json({ error: 'Error saving draft: ' + err.message });
    }
});
