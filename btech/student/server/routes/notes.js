...
try {
    ...
    console.warn('[6TO10 NOTES] SyncManager import warning:', e.message);
}
...
router.post('/', authenticateToken, async (req, res) => {
    try {
        ...
        let book = await get(
            ...
            [bookId, studentId, req.user.id]
        ).catch(() => null);

        if (!book) {
            book = await get("SELECT * FROM books WHERE id = ?", [bookId]).catch(() => null);
            if (!book) {
        ...
        console.log(`[6TO10 NOTES] Book ID: ${bookId}`);

        const result = await run(
            "INSERT INTO notes (firebase_uid, note_id, book_id, title, rule_type, content, drawing_data, sync_status) VALUES (?, ?, ?, ?, ?, ?, ?, 'pending')",
            [firebaseUid, noteIdString, bookId, title || 'Untitled Note', validRule, content || '', drawing_data || '']
        );
